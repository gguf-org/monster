...
import { CoderIgnoreController } from "@core/ignore/CoderIgnoreController"
...
export class Task {
	...
	private coderIgnoreController: CoderIgnoreController
	...
	constructor(params: TaskParams) {
		...
		this.coderIgnoreController = new CoderIgnoreController(cwd)
	...
	public async startTask(task?: string, images?: string[], files?: string[]): Promise<void> {
		try {
			await this.coderIgnoreController.initialize()
		} catch (error) {
			console.error("Failed to initialize CoderIgnoreController:", error)
	...
	async abortTask() {
		try {
			...
			this.coderIgnoreController.dispose()
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		const coderIgnoreContent = this.coderIgnoreController.coderIgnoreContent
		let coderIgnoreInstructions: string | undefined
		if (coderIgnoreContent) {
			coderIgnoreInstructions = formatResponse.coderIgnoreInstructions(coderIgnoreContent)
		}
	...
	async getEnvironmentDetails(includeFileDetails: boolean = false) {
		...
		const allowedVisibleFiles = this.coderIgnoreController
		...
		// Filter paths through clineIgnoreController
		const allowedOpenTabs = this.coderIgnoreController
		...
		if (includeFileDetails) {
			...
			if (isDesktop) {
				...
				const result = formatResponse.formatFilesList(this.cwd, files, didHitLimit, this.coderIgnoreController)
				details += result
			}
