...
import type { CoderIgnoreController } from "@core/ignore/CoderIgnoreController";
...
export class ToolValidator {
	constructor(private readonly coderIgnoreController: CoderIgnoreController) {}
	 ...
	 */
	checkCoderIgnorePath(relPath: string): ValidationResult {
		const accessAllowed = this.coderIgnoreController.validateAccess(relPath)
		if (!accessAllowed) {
			return {
				ok: false,
				error: `Access to path '${relPath}' is blocked by .coderignore settings.`,
			}
		}
		return { ok: true }
	}
