...
export class ReadFileToolHandler implements IFullyManagedTool {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		const accessValidation = this.validator.checkCoderIgnorePath(relPath!)
		if (!accessValidation.ok) {
			await config.callbacks.say("coderignore_error", relPath)
			return formatResponse.toolError(formatResponse.coderIgnoreError(relPath!))
		}
		...
		if (await config.callbacks.shouldAutoApproveToolWithPath(block.name, relPath)) {
			...
			const notificationMessage = `Coder wants to read ${getWorkspaceBasename(absolutePath, "ReadFileToolHandler.notification")}`
