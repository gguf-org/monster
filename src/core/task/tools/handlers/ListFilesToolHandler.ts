...
export class ListFilesToolHandler implements IFullyManagedTool {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		const accessValidation = this.validator.checkCoderIgnorePath(relDirPath!)
		if (!accessValidation.ok) {
			await config.callbacks.say("coderignore_error", relDirPath)
			return formatResponse.toolError(formatResponse.coderIgnoreError(relDirPath!))
		}

		// Execute the actual list files operation
		const [files, didHitLimit] = await listFiles(absolutePath, recursive, 200)

		const result = formatResponse.formatFilesList(absolutePath, files, didHitLimit, config.services.coderIgnoreController)
		...
		if (await config.callbacks.shouldAutoApproveToolWithPath(block.name, relDirPath)) {
			...
			const notificationMessage = `Coder wants to view directory ${getWorkspaceBasename(absolutePath, "ListFilesToolHandler.notification")}/`
