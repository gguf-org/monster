...
export class ExecuteCommandToolHandler implements IFullyManagedTool {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		const ignoredFileAttemptedToAccess = config.services.coderIgnoreController.validateCommand(actualCommand)
		if (ignoredFileAttemptedToAccess) {
			await config.callbacks.say("coderignore_error", ignoredFileAttemptedToAccess)
			return formatResponse.toolError(formatResponse.coderIgnoreError(ignoredFileAttemptedToAccess))
		}
		...
		if ((!requiresApprovalPerLLM && autoApproveSafe) || (requiresApprovalPerLLM && autoApproveSafe && autoApproveAll)) {
			...
			showNotificationForApproval(
				`Coder wants to execute a command: ${actualCommand}`,
