 ...
 */
export class CoderIgnoreController {
	private cwd: string
	private ignoreInstance: Ignore
	private fileWatcher?: FSWatcher
	coderIgnoreContent: string | undefined

	constructor(cwd: string) {
		this.cwd = cwd
		this.ignoreInstance = ignore()
		this.coderIgnoreContent = undefined
	}
	...
	async initialize(): Promise<void> {
		// Set up file watcher for .coderignore
		this.setupFileWatcher()
		await this.loadCoderIgnore()
	}

	/**
	 * Set up the file watcher for .coderignore changes
	 */
	private setupFileWatcher(): void {
		const ignorePath = path.join(this.cwd, ".coderignore")
		...
		this.fileWatcher.on("error", (error) => {
			console.error("Error watching .coderignore file:", error)
		})
	}

	/**
	 * Load custom patterns from .coderignore if it exists.
	 * Supports "!include <filename>" to load additional ignore patterns from other files.
	 */
	private async loadCoderIgnore(): Promise<void> {
		try {
			...
			if (await fileExistsAtPath(ignorePath)) {
				...
				this.coderIgnoreContent = content
				await this.processIgnoreContent(content)
				this.ignoreInstance.add(".coderignore")
			} else {
				this.coderIgnoreContent = undefined
			}
		} catch (error) {
			// Should never happen: reading file failed even though it exists
			console.error("Unexpected error loading .coderignore:", error)
		}
	}
	...
	private async processIgnoreContent(content: string): Promise<void> {
		...
		const combinedContent = await this.processCoderIgnoreIncludes(content)
	 ...
	 */
	private async processCoderIgnoreIncludes(content: string): Promise<string> {
		let combinedContent = ""
	...
	private async readIncludedFile(includeLine: string): Promise<string | null> {
		...
		if (!(await fileExistsAtPath(resolvedIncludePath))) {
			console.debug(`[CoderIgnore] Included file not found: ${resolvedIncludePath}`)
			return null
		}
	...
	validateAccess(filePath: string): boolean {
		// Always allow access if .coderignore does not exist
		if (!this.coderIgnoreContent) {
			return true
		}
		try {
	...
	validateCommand(command: string): string | undefined {
		// Always allow if no .coderignore exists
		if (!this.coderIgnoreContent) {
			return undefined
		}
