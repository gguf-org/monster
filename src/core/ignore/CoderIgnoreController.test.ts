...
import { CoderIgnoreController } from "./CoderIgnoreController"
import "should"

describe("CoderIgnoreController", () => {
	let tempDir: string
	let controller: CoderIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .coderignore file
		await fs.writeFile(
			path.join(tempDir, ".coderignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new CoderIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .coderignore file", async () => {
			const result = controller.validateAccess(".coderignore")
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .coderignore gracefully", async () => {
			// Create a new controller in a directory without .coderignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new CoderIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .coderignore", async () => {
			await fs.writeFile(path.join(tempDir, ".coderignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .coderignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".coderignore"), ["!include .gitignore", "secret.txt"].join("\n"))
			...
			// "secret.txt" should be ignored as specified directly in .coderignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .coderignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".coderignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".coderignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
