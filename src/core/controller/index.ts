...
import { CoderConfigService } from "@services/config/CoderConfigService"
...
export class Controller {
	...
	private coderConfigService: CoderConfigService
	...
	constructor(readonly context: vscode.ExtensionContext) {
		...
		// Start remote config timer
		this.startRemoteConfigTimer()

		// Initialize CoderConfigService
		this.coderConfigService = CoderConfigService.getInstance()
		// Load config asynchronously and apply settings
		this.loadCoderConfig()
	...
	/**
	 * Load coder.config.json and apply API configuration
	 */
	private async loadCoderConfig() {
		try {
			const config = await this.coderConfigService.loadConfig()
			if (config) {
				console.log("[Controller] Loaded coder.config.json successfully")
				// Apply API configuration from coder.config.json
				const currentApiConfig = this.stateManager.getApiConfiguration()
				const updatedConfig = this.coderConfigService.mapToApiConfiguration(config, currentApiConfig)
				this.stateManager.setApiConfiguration(updatedConfig)
				await this.postStateToWebview()
			} else {
				console.log("[Controller] No coder.config.json found, using default settings")
			}
		} catch (error) {
			console.error("[Controller] Failed to load coder.config.json:", error)
		}
	}
	...
	*/
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		const updatedDictationSettings = {
			...dictationSettings,
			featureEnabled: process.platform === "darwin" || process.platform === "linux" || process.platform === "win32", // Enable dictation on macOS, Linux, and Windows
		}
