...
async function ensureUserInOrgWithRemoteConfig(controller: Controller): Promise<RemoteConfig | undefined> {
	...
	try {
		...
		if (currentActiveOrgId !== organizationId) {
			// Account service has been removed - skip organization switching
			console.log(
				`[fetchRemoteConfig] Account service removed - skipping org switch from ${currentActiveOrgId} to ${organizationId}`,
			)
		}
