...
import { GGMLQuantizationType, gguf, buildGgufHeader, GGUFValueType } from "@gguf/editor"

export async function getWebviewContent(uri: vscode.Uri, searchTerm: string = "") {
...
function formatArray(array: Array<any>) {
	const maxElements = vscode.workspace.getConfiguration("gguf-editor").get<number>("maxArrayElements", 25)
...
function formatTemplate(template: string, values: Record<string, any>): string {
	return template.replace(new RegExp(replaceMark + "\\{(.*?)\\}" + replaceMark, "g"), (_, key) => {
...
export async function saveGGUFMetadata(
	uri: vscode.Uri,
	updatedMetadata: Record<string, any>,
	updatedTensorNames?: Record<string, string>,
	_deletedTensors?: number[],
): Promise<void> {
	// Parse the original file with typed metadata
	const { tensorInfos, tensorDataOffset, littleEndian, typedMetadata, tensorInfoByteRange } = await gguf(uri.fsPath, {
		allowLocalFile: true,
		typedMetadata: true,
	})
	...
	const originalBuffer = Buffer.from(await vscode.workspace.fs.readFile(uri))
	...
	await vscode.workspace.fs.writeFile(newUri, new Uint8Array(arrayBuffer))
