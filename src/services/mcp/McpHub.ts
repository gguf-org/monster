...
import { CoderConfigService } from "../config/CoderConfigService"
export class McpHub {
	...
	constructor(
		...
		coderConfigService?: CoderConfigService,
	) {
		...
		this.coderConfigService = coderConfigService
		// Only watch legacy file if not using CoderConfigService
		if (!this.coderConfigService) {
	...
	private async initializeMcpServers(): Promise<void> {
		// Prioritize CoderConfigService if available
		if (this.coderConfigService) {
			const config = this.coderConfigService.getConfig()
			if (config?.coder.mcpServers) {
				console.log("[McpHub] Loading MCP servers from coder.config.json")
				// Convert CoderConfig MCP servers to McpServerConfig format
				const mcpServers: Record<string, McpServerConfig> = {}
				for (const server of config.coder.mcpServers) {
