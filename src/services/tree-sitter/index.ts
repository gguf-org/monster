...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = coderIgnoreController ? coderIgnoreController.filterPaths(filesToParse) : filesToParse

	for (const filePath of allowedFilesToParse) {
		const definitions = await parseFile(filePath, languageParsers, coderIgnoreController)
		if (definitions) {
...
async function parseFile(
	...
	if (coderIgnoreController && !coderIgnoreController.validateAccess(filePath)) {
		return null
	}
