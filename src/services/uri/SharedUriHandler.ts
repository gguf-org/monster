	...
	public static async handleUri(url: string): Promise<boolean> {
		...
		try {
			switch (path) {
				...
				case "/auth": {
					// Auth has been removed - return false
					Logger.warn("SharedUriHandler: Auth callback received but auth is disabled")
					return false
				}
				case "/auth/oca": {
					// Auth has been removed - return false
					Logger.warn("SharedUriHandler: OCA Auth callback received but auth is disabled")
					return false
				}
				case "/task": {
