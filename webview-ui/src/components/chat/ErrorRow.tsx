...
interface ErrorRowProps {
	...
	errorType: "error" | "mistake_limit_reached" | "diff_error" | "coderignore_error"
...
const ErrorRow = memo(({ message, errorType, apiRequestFailedMessage, apiReqStreamingFailedMessage }: ErrorRowProps) => {
		...
		switch (errorType) {
			...
			case "coderignore_error":
				return (
					<div className="flex flex-col p-2 rounded text-xs opacity-80 bg-quote text-foreground">
						<div>
							Coder tried to access <code>{message.text}</code> which is blocked by the <code>.coderignore</code>
							file.
						</div>
					</div>
				)
	...
	// For diff_error and coderignore_error, we don't show the header separately
	if (errorType === "diff_error" || errorType === "coderignore_error") {
